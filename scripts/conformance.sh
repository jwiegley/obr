#!/usr/bin/env bash
# Conformance test runner - compares obr and bd outputs for parity.
#
# Usage:
...
if ! BD_PATH=$(find_bd); then
	...
	error "Conformance tests require both obr (Rust) and bd (Go) binaries."
...
# Ensure obr build is up to date
log "Building obr binary..."
cd "$PROJECT_ROOT"
cargo build --release --quiet 2>/dev/null || cargo build --release

BR_PATH="${BR_BINARY:-$(cargo metadata --format-version=1 2>/dev/null | jq -r '.target_directory')/release/obr}"
if [[ ! -x "$BR_PATH" ]]; then
	BR_PATH="$PROJECT_ROOT/target/release/obr"
fi
log "Using obr at: $BR_PATH"
BR_VERSION=$("$BR_PATH" version 2>/dev/null | head -1 || echo "unknown")
log "obr version: $BR_VERSION"
...
log "obr: $BR_VERSION | bd: $BD_VERSION"
log "Artifacts: $ARTIFACTS_DIR/conformance"
log "============================================"
