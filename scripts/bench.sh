#!/usr/bin/env bash
# Benchmark runner - measures obr performance with timing and optional comparisons.
#
# Usage:
...
#   scripts/bench.sh --compare          # Compare obr vs bd performance
...
#   BENCH_DATASET=obr  Dataset to use for benchmarks
#
# Output:
...
log "Building obr binary (release)..."
cd "$PROJECT_ROOT"
...
run_criterion_benchmarks() {
	...
	# Run criterion benchmarks ($baseline_args is a flag+value pair; splitting intended)
...
run_bd_comparison() {
	log "Running obr vs bd performance comparison..."
	...
	for op in "${ops[@]}"; do
		...
		# Time obr ($op carries multi-word subcommands; splitting intended)
		local br_start br_end br_time bd_start bd_end bd_time speedup
		br_start=$(date +%s.%N)
		# shellcheck disable=SC2086
		...
		log "  $op: obr=${br_time}s bd=${bd_time}s speedup=${speedup}x"
	done
