# obr sync Safety Invariants and Non-Goals

> Precise, testable invariants for obr sync operations.
> Each invariant is phrased for direct assertion and logging.

---

## 1. Non-Goals (What obr sync Will NEVER Do)

These are explicit design exclusions. obr sync is intentionally less invasive than its Go predecessor.
...
| NG-3 | **Run as daemon or background process** | Simple CLI only; no persistent state outside .obr/ |
| NG-4 | **Auto-commit changes** | Every Git operation remains user-controlled; `obr vcs-status` is read-only |
| NG-5 | **Delete files outside .obr/** | Minimal filesystem footprint; data safety by confinement |
| NG-6 | **Create files outside .obr/** (without explicit opt-in) | Path confinement by default |
| NG-7 | **Modify files outside .obr/** | All mutations confined to project metadata |
...
|----|------|-----------|---------------|
| PC-1 | CRITICAL | All file writes occur within `.obr/` directory OR an explicitly user-specified JSONL path | Unit test: mock filesystem, assert no writes outside allowlist |
| PC-2 | HIGH | If `OBR_JSONL` env var is set, it MUST be validated and logged before use | Unit test: verify validation function is called; log assertion |
| PC-3 | HIGH | All paths are canonicalized before I/O operations | Unit test: verify symlink resolution doesn't escape .obr/ |
...
| PC-RECOVERY | MEDIUM | Sync invocations may *transitively* cause storage recovery flows in `src/config/mod.rs::backup_database_family_for_recovery` to write under `.obr/recovery/` with filenames `<original-name>.<stamp>.<suffix>` where `<suffix>` ∈ {`bak`, `rebuild-failed`, `truncated-wal`}. Recovery flow has its own path validation (separate from sync's `validate_sync_path`); the two are deliberately decoupled because recovery is a storage concern, not a sync concern. Tests that take a workspace-wide snapshot during sync MUST recognize these as legitimate side-effect writes. | Unit + e2e tests in `tests/e2e_sync_git_safety.rs::is_allowed_sync_file` recognize the three suffixes; arbitrary other contents in `.obr/recovery/` are still rejected. |
...
| AR-9 | HIGH | Receipt JSON Schema and command envelope are discoverable through `obr schema` | Schema catalog and enum-domain tests |
...
Guards are named, not line-numbered: every number in this table had drifted
by thousands of lines and pointed at unrelated code, which is worse than no
pointer at all. Find each with `git grep -n 'fn <name>'`.

| Invariant | Implementation Guard | Location |
|-----------|---------------------|----------|
| PC-1 | `validate_sync_path()` | `src/sync/path.rs` |
| PC-2 | `require_valid_sync_path()` | `src/sync/path.rs` |
| PC-3 | Path canonicalization inside `validate_sync_path()` | `src/sync/path.rs` |
| PC-4 | `is_allowed_export_temp_name()` (same-directory temp names) | `src/sync/path.rs` |
| DL-1 | `count_issues_in_jsonl()` check | `src/sync/mod.rs` |
| DL-2 | `get_issue_ids_from_jsonl()` diff check | `src/sync/mod.rs` |
| IV-1 | `ensure_no_conflict_markers()` | `src/sync/mod.rs` |
| AW-1 | Staged temp file + `durable_rename()` | `src/sync/mod.rs`, `src/util/mod.rs` |
| AW-2 | `flush()` + `sync_all()` before publication | `src/sync/mod.rs` |
| DL-3 | Tombstone check in import | `src/sync/mod.rs` import logic |
| D-SURFACE | `is_workspace_surface_path()` (the tracked surface is in-allowlist) | `src/sync/path.rs` |
...
| Use external JSONL path | `OBR_JSONL` env var | Future: `--allow-external-jsonl` flag |
...
1. **Path Allowlist Validation** (Priority 1)
   - Validate `OBR_JSONL` must be within `.obr/` or require explicit flag
...
|----|----------|-----------|
| PC-1 | Path Confinement | All file writes within `.obr/` or explicit JSONL |
...
| ID | Category | Invariant |
|----|----------|-----------|
| PC-2 | Path Confinement | OBR_JSONL env var validated and logged |
