...
obr init --prefix md
obr create "Test issue" --type task --priority 1
...
OUTPUT=$(NO_COLOR=1 obr list 2>&1)
...
OUTPUT=$(obr list --quiet 2>&1 || true)
...
for cmd in "list" "ready" "blocked" "stats"; do
	OUTPUT=$(obr $cmd --no-color 2>&1 || true)
	if has_ansi "$OUTPUT"; then
...
OUTPUT=$(NO_COLOR=1 obr list --json 2>&1)
if echo "$OUTPUT" | jq -e '.' >/dev/null 2>&1; then
