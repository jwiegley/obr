...
log "obr version: $(obr version --json 2>/dev/null | jq -r '.version' || obr version)"
...
obr init --prefix e2e 2>&1
if [[ -f .obr/obr.db ]]; then
...
obr create "High priority bug" --type bug --priority 0
obr create "Medium task" --type task --priority 2
obr create "Low feature" --type feature --priority 3
ISSUE_COUNT=$(obr list --json | jq '.issues | length')
...
log "--- JSON mode:"
obr list --json | jq -c '.issues[] | {id, title}' | head -5
log "--- Plain mode (--no-color):"
PLAIN_OUTPUT=$(obr list --no-color 2>&1)
...
obr dep add "$ID2" "$ID1"
obr blocked --no-color | head -5
obr blocked --json | jq -c 'length' >/dev/null
log "✓ Blocked command works"

# Test 8: Stats command
log_section "TEST 8: Stats command"
obr stats --no-color | head -10
obr stats --json 2>/dev/null | jq -e '.summary.total_issues' >/dev/null
