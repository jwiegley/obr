...
#   2   prerequisite missing (obr binary, git)
...
if [[ -n "${OBR_BIN:-${BR_BIN:-}}" ]]; then
	...
	log "ERROR: obr binary not found (set BR_BIN= or build with cargo build --release)"
	exit 2
fi
...
# Phase 1: obr init with custom prefix
log "Phase 1: obr init --prefix myproj"
"$BR" init --prefix myproj >/dev/null 2>&1 || fail "obr init"

# Phase 2: obr create --slug
log "Phase 2: obr create --slug 'Fix Login Flow' (mixed case + spaces)"
...
# Phase 3: obr show --json round-trip
log "Phase 3: obr show $SLUG_ID --json"
SHOW_OUT=$("$BR" show "$SLUG_ID" --json 2>&1) || fail "obr show failed"
...
# Phase 4: obr update
log "Phase 4: obr update $SLUG_ID --priority 0"
"$BR" update "$SLUG_ID" --priority 0 >/dev/null 2>&1 || fail "obr update failed"
...
log "Phase 5b: obr orphans --json"
ORPHANS_OUT=$("$BR" orphans --json 2>&1) || fail "obr orphans failed"
...
# Phase 6: obr close
log "Phase 6: obr close $SLUG_ID --reason 'PASS lifecycle test'"
"$BR" close "$SLUG_ID" --reason "PASS lifecycle test" >/dev/null 2>&1 || fail "obr close failed"
...
log "Phase 7: obr orphans --json (should not include closed bead)"
