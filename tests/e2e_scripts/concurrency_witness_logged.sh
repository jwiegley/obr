...
#
# Drives N concurrent `obr list` reads + 1 `obr update` write against a shared
# workspace, captures stderr per-process, asserts the post-update state is
# observable from a final `obr show <id> --json`, and emits a structured
...
# shellcheck disable=SC2329  # standard harness helper; invoked in some variants
fail() {
	log "FAIL: $*"
	exit 1
}
...
run_iteration() {
	...
	# init + create + flush
	"$BR" init --prefix obr >/dev/null 2>&1 || {
		emit_event "init_fail" 0 1 "obr init failed" "$iter"
		cd /
		rm -rf "$work"
		return 1
	}
	local create_out
	create_out=$("$BR" create "iter $iter target" -t task --no-auto-flush 2>&1) || {
		emit_event "create_fail" 0 1 "obr create failed" "$iter"
		cd /
		rm -rf "$work"
		return 1
	}
	local target_id
	target_id=$(echo "$create_out" | grep -oE 'obr-[a-z0-9-]+' | head -1)
	[[ -n "$target_id" ]] || {
		emit_event "id_extract_fail" 0 1 "$create_out" "$iter"
		cd /
		rm -rf "$work"
		return 1
	}
	...
	# Spawn N concurrent readers (obr list --json)
	local reader_pids=()
	for r in $(seq 1 "$NUM_READERS"); do
		(
			"$BR" list --json >"/tmp/concur_reader_${LOG_TS}_${iter}_${r}.out" 2>"/tmp/concur_reader_${LOG_TS}_${iter}_${r}.err"
			emit_event "reader_done" "$$" $? "" "$iter"
		) &
		reader_pids+=($!)
	done

	# One concurrent writer
	"$BR" update "$target_id" --priority 0 >"/tmp/concur_writer_${LOG_TS}_${iter}.out" 2>"/tmp/concur_writer_${LOG_TS}_${iter}.err" || {
		emit_event "writer_fail" 0 1 "" "$iter"
		cd /
		rm -rf "$work"
		return 1
	}
