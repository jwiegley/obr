# `obr doctor` Real-World Fixture Suite (Phase 9)

This directory holds runnable fixtures that exercise `obr doctor` against real
corrupt workspaces end-to-end:

```
corrupt.sh  →  obr doctor --json  →  assert.sh (Stage A: detect)
            →  obr doctor --repair --json  →  assert.sh (Stage B: post-repair)
            →  obr doctor undo <first-repair-run-id> --json (best-effort round-trip)
```

Each fixture directory contains:

- `corrupt.sh <target_dir>` — deterministic recipe to plant the failure inside
  a fresh tempdir. Receives `TOOL_BIN` env (path to the `obr` binary). Must
...
- `assert.sh <target_dir> <stage>` — invoked in two stages:
    - `assert.sh DIR detect` — runs `obr doctor --json` and asserts the
      planted failure surfaces in the expected check name + status.
    - `assert.sh DIR post_repair` — invoked after `obr doctor --repair`; asserts
...
`obr doctor undo latest` only restores files the **chokepoint** (`mutate()`)
...
coverage. The `gitignore_leaking_obr` fixture *does* round-trip fully and
...
`obr doctor --repair --json` path.

Per AGENTS.md: no `Command::new("git")` from runtime `obr` code; the fixture
