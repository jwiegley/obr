#!/usr/bin/env bash
# Fixture assertions: labels_orphans
#
# Pass-5 cycle 35: fm-caches_indexes-labels-orphans graduates from
# undetected (Tier D) to auto-fixed (Tier A). --repair issues a
# surgical DELETE via Op::DbExec with predicate-based snapshotting.
# JSONL bytes byte-unchanged throughout.
...
case "$stage" in
  detect)
    ...
    echo "$out" | jq -e '
      .checks[] | select(.name == "labels.orphans")
      | select(.status == "warn" or .status == "error")
    ' >/dev/null || {
      echo "ASSERT FAIL[$stage]: labels.orphans not flagged" >&2
      echo "$out" | jq '.checks[] | select(.name == "labels.orphans")' >&2
      exit 1
    }
    count=$(orphan_count)
    if [ "$count" != "1" ]; then
  ...
  post_repair)
    ...
    if [ -f "$target_dir/_diag/repair.json" ]; then
      jq -e '
        .repaired == true
        and (.recovery_audit.applied_actions | index("labels_orphans_pruned"))
      ' "$target_dir/_diag/repair.json" >/dev/null || {
        echo "ASSERT FAIL[$stage]: repair output did not report labels_orphans_pruned" >&2
    ...
    status=$(echo "$redetect" | jq -r '.checks[] | select(.name == "labels.orphans") | .status' 2>/dev/null || echo "")
    if [ "$status" != "ok" ] && [ -n "$status" ]; then
      echo "ASSERT FAIL[$stage]: labels.orphans still '$status' after repair" >&2
      exit 1
    fi
