...
# `obr create`, then appends ~33MB of zero padding to inflate it past
...
# wal_size warn. `obr init` writes a template with `*.lock` and
...
# Force at least one DB write so SQLite materialises obr.db-wal
...
dd if=/dev/zero bs=1048576 count=33 status=none >>.obr/obr.db-wal

size_bytes=$(wc -c <.obr/obr.db-wal)
