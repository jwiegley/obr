...
# invoking `obr doctor --json` (which would remove it before
...
case "$stage" in
detect)
	# Verify the planted state is on disk. Intentionally NO `obr doctor`
	# call here — see header comment.
	[ -f .obr/obr.db-wal ] || {
		echo "ASSERT FAIL[$stage]: planted WAL sidecar missing" >&2
		exit 1
	}
	size=$(wc -c <.obr/obr.db-wal)
	...
	[ -f .obr/obr.db ] || {
		echo "ASSERT FAIL[$stage]: obr.db missing" >&2
		exit 1
	}
	;;
...
post_repair)
	...
	"$tool_bin" list --json >/dev/null 2>&1 || {
		echo "ASSERT FAIL[$stage]: obr list failed post-checkpoint" >&2
		exit 1
	}
...
post_undo)
	...
	"$tool_bin" list --json >/dev/null 2>&1 || {
		echo "ASSERT FAIL[$stage]: obr list failed post-undo" >&2
		exit 1
	}
	;;
