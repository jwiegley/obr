...
grep -qxF ".write.lock" .obr/.gitignore 2>/dev/null || printf "\n.write.lock\n" >> .obr/.gitignore
...
cp .obr/issues.jsonl .obr/merge.base.jsonl

sqlite3 .obr/obr.db \
  "INSERT OR REPLACE INTO dirty_issues(issue_id, marked_at) VALUES('$issue_id', '2026-01-01T00:00:00Z')"

sha256sum .obr/issues.jsonl | awk '{print $1}' > .fixture_jsonl_sha256
