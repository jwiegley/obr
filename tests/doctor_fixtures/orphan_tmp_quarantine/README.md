...
- **FM**: `fm-state_files-orphan-tmp-files` (P2) — `.obr/*.tmp`
  or `.obr/*.tmp.<digits>` files older than 1 hour. Interrupted
...
- **Repair contract**: SAFETY — `--repair` renames each orphan tmp
  into `<run-dir>/quarantine/.obr/` via the `mutate()` chokepoint
...
- **Round-trip**: plant `.obr/foo.tmp` with mtime 2h ago →
  detect warn → `--repair` quarantines the file → re-detect ok →
  `doctor undo` restores `.obr/foo.tmp` byte-deterministically.
