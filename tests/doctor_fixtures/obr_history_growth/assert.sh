#!/usr/bin/env bash
# Fixture assertions: obr_history_growth
...
history_count() {
	find .obr/history -maxdepth 1 -type f -name 'issues.*.jsonl' 2>/dev/null | wc -l | tr -d '[:space:]'
}

assert_seed_history_intact() {
	if [ ! -f ".obr/history/$seed_history" ]; then
		echo "ASSERT FAIL[$stage]: seed history snapshot vanished: $seed_history" >&2
		exit 1
	fi
	if ! grep -q '"id":"bd-history-001"' ".obr/history/$seed_history"; then
...
detect)
	...
	echo "$out" | jq -e '
      .checks[] | select(.name == "obr_history.size")
      | select(.status == "warn")
      | select(.details.snapshot_count >= 105)
      | select(.details.threshold == 100)
    ' >/dev/null || {
		echo "ASSERT FAIL[$stage]: obr_history.size did not warn with expected details" >&2
		echo "$out" | jq '.checks[] | select(.name == "obr_history.size")' >&2
		exit 1
	}
	assert_seed_history_intact
	;;
