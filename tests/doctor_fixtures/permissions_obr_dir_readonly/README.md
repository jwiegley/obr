# permissions_obr_dir_readonly

- **FM**: `fm-permissions-obr-dir-readonly` (P0)
- **Subsystem**: permissions
- **Detect**: `permissions.obr_dir` check goes to `warn` enumerating
...
- **Repair contract**: SAFETY — detect-only. The doctor NEVER `chmod`s
  the operator's `.obr/` files. The safe action is to surface the
...
`.obr/issues.jsonl` to 0444 to trigger the detector. assert.sh
