...
- **Detector contract**: read-only `obr doctor --json` must not mutate `.obr/`.
- **Detect**: plants a malformed 20-byte `.obr/obr.db` while leaving the
  authoritative JSONL and a WAL sidecar in place. The old sqlite3 fallback used
  to create `.obr/obr.db-shm` while reporting the malformed database.
- **Repair contract**: normal `obr doctor --repair` may rebuild the database
  from JSONL. The invariant under test is that every subsequent read-only
  `doctor --json` invocation preserves the current `.obr/` file list and
  bytes exactly.
