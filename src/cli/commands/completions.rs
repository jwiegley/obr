...
//! obr completions bash
//!
//! # Generate zsh completions to a file
//! obr completions zsh -o ~/.zsh/completions/_obr
//!
//! # Generate fish completions
//! obr completions fish > ~/.config/fish/completions/obr.fish
//! ```
...
pub fn print_install_instructions(shell: ShellType) {
    match shell {
        ShellType::Bash => {
            ...
            eprintln!("obr completions bash > ~/.local/share/bash-completion/completions/obr");
            eprintln!("\n# Option 2: System-wide (requires sudo)");
            eprintln!("sudo obr completions bash > /etc/bash_completion.d/obr");
        ...
        ShellType::Zsh => {
            ...
            eprintln!("obr completions zsh > ~/.zsh/completions/_obr");
            eprintln!("# Add to ~/.zshrc: fpath=(~/.zsh/completions $fpath)");
            eprintln!("\n# Option 2: Oh My Zsh");
            eprintln!("obr completions zsh > ~/.oh-my-zsh/completions/_obr");
            eprintln!("\n# Then restart your shell or run: exec zsh");
        }
        ShellType::Fish => {
            eprintln!("\n# Installation instructions for fish:");
            eprintln!("mkdir -p ~/.config/fish/completions");
            eprintln!("obr completions fish > ~/.config/fish/completions/obr.fish");
        ...
        ShellType::PowerShell => {
            ...
            eprintln!("obr completions powershell | Out-String | Invoke-Expression");
        }
        ShellType::Elvish => {
            eprintln!("\n# Installation instructions for elvish:");
            eprintln!("mkdir -p ~/.elvish/lib");
            eprintln!("obr completions elvish > ~/.elvish/lib/obr.elv");
            eprintln!("# Add to ~/.elvish/rc.elv: use obr");
        }
    }
}
