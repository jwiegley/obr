...
//!
//! Every `obr doctor --repair` run lays down an artifact directory:
//!
//! ```text
...
pub const ENV_RUNS_DIR: &str = "OBR_DOCTOR_RUNS_DIR";
...
fn create_run_dir_with_actions_file(
    ...
    // any benefit. Test fixtures, CI sandboxes, and `obr doctor undo`
...
/// Public so the doctor checks that report ON the runs root ask this module
/// where it is, instead of rebuilding the path themselves. Both of them used to
/// rebuild it and both got it wrong: one stat'd `.doctor` rather than
/// `.doctor/runs`, and neither honoured the `OBR_DOCTOR_RUNS_DIR` redirect that
/// creation here obeys — so the two states that actually make `--repair` die
/// with EACCES both reported ok.
#[must_use]
pub fn runs_root_for(repo_root: &Path) -> PathBuf {
...
///    `obr doctor undo`, fixtures) cause `create_run_dir` to skip this
...
fn ensure_doctor_in_gitignore(repo_root: &Path) -> Result<(), BeadsError> {
    ...
    new_contents.push_str("# obr doctor per-run artifacts\n");
...
/// cp, mv) so it is recoverable even when the `obr` binary itself is
/// broken.
///
/// # Errors
///
...
pub fn write_undo_sh(run: &RunDir) -> Result<(), BeadsError> {
    ...
    let script = format!(
        r#"#!/usr/bin/env bash
# obr doctor undo — pure-bash fallback for run {run_id}
#
# Replays {{actions_jsonl}} in reverse, restoring the verbatim backups
# under {{backups_dir}}. Requires: bash, jq, cp, mv.
#
# This script is generated by obr; do NOT hand-edit unless the live obr
# binary is broken.
set -euo pipefail
...
tac "${{actions}}" | while read -r line; do
  ...
  case "${{op}}" in
    ...
    rename)
      ...
      ;;
    db_exec|db_migrate)
      echo "[warn] cannot undo ${{op}} from bash — re-run obr doctor undo" >&2
      ;;
    *)
...
mod tests {
    ...
    /// sandboxes, and `obr doctor undo` (which builds a fresh run-dir
