...
class Obr < Formula
  ...
  # `0.2.22+1` is the upstream beads_rust release this fork tracks, plus the
  # fork generation since it. Homebrew's Version understands the whole string
  # and orders it correctly (0.2.22+2 > 0.2.22+1 > 0.2.22), unlike the semver
  # SPEC and any requirement matcher that follows it, which ignore build
  # metadata for precedence.
  version "0.2.22+1"
  # MIT; see the LICENSE file for the upstream rider retained verbatim.
  license "MIT"
  ...
  # before uploading and the file on the release is `obr-0.2.22.1-*`.
